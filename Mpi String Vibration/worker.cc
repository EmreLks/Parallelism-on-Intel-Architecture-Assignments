...
float * simulate(const float alpha, const long n_segments, const int n_steps, float *d_buf1, float *d_buf2, const int rank, const int world_size, const long segments_per_process) {
  ...
  for(int t = 0; t < n_steps; t++)
  {
    ...
    for(long i = start_segment; i < last_segment; i++)
	{
	  ...
	  // The algorithm calls for d(i, t-1) here, 
	  // but that is currently contained in d_t1
    }
	
	float* temp = d_t1; d_t1 = d_t; d_t=temp; // swap buffers
		
	/////////////////
	int lowerNeighbor = rank - 1, upperNeighbor = rank + 1;
    ...
    {
	    MPI_Sendrecv(&d_t[start_segment]    , 1, MPI_FLOAT, lowerNeighbor, 0, 
                     &d_t[start_segment - 1], 1, MPI_FLOAT, lowerNeighbor, 0,
					 MPI_COMM_WORLD, &stat2);
    }
	// First process
	else if (rank == 0)
	{
	    MPI_Sendrecv(&d_t[last_segment - 1], 1, MPI_FLOAT, upperNeighbor, 0, 
                     &d_t[last_segment]    , 1, MPI_FLOAT, upperNeighbor, 0,
					 MPI_COMM_WORLD, &stat2);
	}
	// Others
	else
